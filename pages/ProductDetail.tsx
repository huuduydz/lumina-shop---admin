...
import { Battery, Mic, RefreshCw, ShieldCheck, ShoppingBag, Star, Truck, Zap } from 'lucide-react';
...
const ProductDetail = () => {
  ...
  const productColors = useMemo(() => getColorOptions(product), [product]);
  const productSizes = useMemo(() => getSizeOptions(product), [product]);
  ...
  const relatedProducts = useMemo(
    () =>
      products
        ...
        .sort((first, second) => {
          const firstAvailable = first.stockStatus !== 'Out of Stock' ? 1 : 0;
          const secondAvailable = second.stockStatus !== 'Out of Stock' ? 1 : 0;
          return secondAvailable - firstAvailable || second.rating - first.rating || second.reviews - first.reviews;
        })
        .slice(0, 4),
  ...
  const [selectedSize, setSelectedSize] = useState(productSizes[0].label);
  ...
  const selectedColorOption =
    productColors.find((color) => color.name === selectedColor) || productColors[0];
  const selectedSizeOption =
    productSizes.find((size) => size.label === selectedSize) || productSizes[0];
  const variantPricing = calculateVariantPrice(product, selectedColor, selectedSize);

  const handleAddToCart = () => {
    addToCart(product, quantity, selectedColor, selectedSize, variantPricing);
  };
  ...
  const handleQuantityChange = (value: number) => {
    let nextQuantity = Math.max(1, value);
    ...
    <div className="flex-1 px-4 py-6 md:px-10 md:py-8 lg:px-40 lg:py-12 bg-surface">
      <div className="max-w-[1280px] mx-auto flex flex-col gap-8">
        <nav className="flex flex-wrap gap-2 text-sm">
          ...
          <Link to="/shop" className="text-slate-500 font-medium hover:text-primary transition-colors">
            {product.category}
          </Link>
          <span className="text-slate-400 font-medium">/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          <div className="flex flex-col-reverse md:flex-row gap-4">
            <div className="flex md:flex-col gap-3 overflow-x-auto md:overflow-y-auto md:h-[500px] no-scrollbar py-1">
              {(product.thumbnails?.length ? product.thumbnails : [product.image]).map((thumb, index) => (
                <button
                  key={index}
          ...
          <div className="flex flex-col">
            <div className="border-b border-slate-200 pb-6 mb-6">
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="flex text-yellow-400 gap-0.5">
                  {[...Array(5)].map((_, index) => (
                    <Star
                      key={index}
                      className={`size-5 ${index < Math.floor(product.rating) ? 'fill-current' : 'text-slate-200'}`}
                    />
                  ))}
                </div>
              ...
              <div className="flex items-baseline gap-4">
                <span className="text-3xl font-bold text-slate-900">${variantPricing.finalPrice.toFixed(2)}</span>
                {variantPricing.finalOriginalPrice && (
                  <span className="text-lg text-slate-400 line-through">
                    ${variantPricing.finalOriginalPrice.toFixed(2)}
                  </span>
                )}
              </div>
              {(variantPricing.colorExtra > 0 || variantPricing.sizeExtra > 0) && (
                <p className="mt-3 text-sm text-primary font-medium">
                  Base ${variantPricing.basePrice.toFixed(2)}
                  {variantPricing.colorExtra > 0
                    ? ` + color ${formatAdditionalPrice(variantPricing.colorExtra)}`
                    : ''}
                  {variantPricing.sizeExtra > 0
                    ? ` + model ${formatAdditionalPrice(variantPricing.sizeExtra)}`
                    : ''}
                </p>
              )}
            </div>
            ...
            <div className="space-y-6 mb-8">
              <div>
                <span className="block text-sm font-semibold text-slate-900 mb-3">
                  {t('product.color')} <span className="font-normal text-slate-500">{selectedColor}</span>
                </span>
                <div className="flex flex-wrap gap-3">
                  {productColors.map((color) => (
                    <button
                      ...
                      title={color.name}
                      className={`flex items-center gap-3 rounded-2xl border px-3 py-2 shadow-sm ring-2 ring-offset-2 transition-all ${
                        selectedColor === color.name
                          ? 'ring-primary border-primary/30'
                          : 'ring-transparent border-slate-200'
                      }`}
                    >
                      <span
                      ...
                      <span className="text-left">
                        <span className="block text-sm font-semibold text-slate-900">{color.name}</span>
                        <span className="block text-xs text-slate-500">
                          {formatAdditionalPrice(color.additionalPrice)}
                        </span>
                      </span>
                    </button>
                  ))}
              ...
              <div>
                <span className="block text-sm font-semibold text-slate-900 mb-3">
                  Model / Size <span className="font-normal text-slate-500">{selectedSize}</span>
                </span>
                <div className="flex flex-wrap gap-3">
                  {productSizes.map((size) => (
                    <button
                      ...
                      className={`px-4 py-3 rounded-xl border text-sm font-semibold min-w-[112px] ${
                    ...
                    >
                      ...
                      <span
                        className={`block text-xs mt-1 ${
                          selectedSize === size.label ? 'text-slate-300' : 'text-slate-500'
                        }`}
                      >
                        {formatAdditionalPrice(size.additionalPrice)}
                      </span>
                    </button>
                  ))}
                </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                ...
                <span className="ml-2 font-semibold text-slate-900">
                  {selectedColorOption.name} / {selectedSizeOption.label}
                </span>
                <span className="ml-2 text-primary font-semibold">
                  +${(selectedColorOption.additionalPrice + selectedSizeOption.additionalPrice).toFixed(2)}
                </span>
              </div>

              <p className="text-slate-600 leading-relaxed">
                {product.description ||
                  'Experience sound like never before with our latest driver technology. Immerse yourself in pure audio bliss with active noise cancellation and a comfortable, ergonomic fit designed for all-day wear.'}
              </p>
              ...
              {product.stockStatus !== 'Out of Stock' && (
                <div className="flex items-center gap-2">
                  <p className="text-sm text-slate-500">
                    {t('product.stock')}{' '}
                    <span className={`font-bold ${product.stockQuantity < 10 ? 'text-orange-600' : 'text-slate-900'}`}>
            ...
            <div className="bg-slate-50 rounded-xl p-5 border border-slate-200">
              <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
                <div className="flex items-center justify-between border border-slate-200 rounded-lg bg-white w-full sm:w-32 h-12">
                  <button
                    ...
                    className="px-3 h-full hover:bg-slate-50 rounded-l-lg text-slate-500 font-bold text-lg"
                  >
                    -
                  </button>
                  <input
              ...
              <div className="mt-4 flex items-center justify-center gap-6 text-xs text-slate-500">
                <span className="flex items-center gap-1">
                  <Truck className="size-4" /> {t('home.features.shipping')}
                </span>
                <span className="flex items-center gap-1">
                  <ShieldCheck className="size-4" /> 2 Year Warranty
                </span>
                <span className="flex items-center gap-1">
                  <RefreshCw className="size-4" /> {t('home.features.returns')}
                </span>
              </div>
            </div>
          </div>
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-6 border-t border-slate-200 pt-10">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3 mb-6">
              <div>
                <h2 className="text-2xl font-bold text-slate-900">San pham cung loai</h2>
                <p className="text-slate-500">
                  Cac san pham cung danh muc {product.category} dang duoc xem nhieu.
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
              {relatedProducts.map((relatedProduct) => {
                const defaultColor = getColorOptions(relatedProduct)[0];
                const defaultSize = getSizeOptions(relatedProduct)[0];
                const defaultPricing = calculateVariantPrice(
                  relatedProduct,
                  defaultColor.name,
                  defaultSize.label
                );
                ...
                return (
                  <div key={relatedProduct.id} className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                    <Link to={`/product/${relatedProduct.id}`} className="block aspect-[4/3] bg-slate-100 overflow-hidden">
                      <img
                    ...
                    <div className="p-4">
                      ...
                      <Link
                        ...
                        className="block text-base font-bold text-slate-900 hover:text-primary transition-colors line-clamp-2"
                      >
                        {relatedProduct.name}
                      </Link>
                      <div className="flex items-center gap-2 mt-3">
                        <span className="text-lg font-bold text-primary">
                          ${defaultPricing.finalPrice.toFixed(2)}
                        </span>
                        {defaultPricing.finalOriginalPrice && (
                          <span className="text-sm text-slate-400 line-through">
                            ${defaultPricing.finalOriginalPrice.toFixed(2)}
                          </span>
                        )}
                      </div>
                      <button
                        ...
                        disabled={relatedProduct.stockStatus === 'Out of Stock'}
                        className="mt-4 w-full rounded-xl bg-slate-900 text-white font-semibold py-2.5 hover:bg-primary transition-colors disabled:bg-slate-300 disabled:cursor-not-allowed"
                      >
                        {relatedProduct.stockStatus === 'Out of Stock' ? 'Het hang' : 'Them nhanh'}
                      </button>
                    </div>
                  </div>
                );
